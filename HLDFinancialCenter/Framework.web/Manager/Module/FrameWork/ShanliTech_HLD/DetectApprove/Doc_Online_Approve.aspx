<%@ Page Language="C#" MasterPageFile="~/Manager/MasterPage/PageTemplate.Master" AutoEventWireup="True" CodeBehind="Doc_Online_Approve.aspx.cs" Inherits="FrameWork.web.Manager.Module.ShanliTech_HLD.DetectApprove.Doc_Online_Approve" %>
...
<link rel="stylesheet" href="../../../Css/Site_Css.css" type="text/css" />
    ...
    <FrameWorkWebControls:HeadMenuWebControls ID="HeadMenuWebControls1" runat="server"
         HeadOPTxt="在线批准证书" HeadTitleTxt="在线批准证书">
                        ...
                        <tr>
                            <td>
                                ...
                                <%
                                    ...
                                    { 
                                        %>
                                <input type="button" value="批准通过并签名" id="Button10" />
                                <input type="button" value="批准驳回" id="Button4" />
                                <%} %>
                                </div>
                                </td>
                        </tr>
...
$('#Button4').click(function(){
    HideWord();
    showPopWin('批准驳回原因说明','../../../inc/NoPass.aspx?CMD=Approve&DetectID=<%=DetectID %>&UserID=<%=userid %>&rand'+rand(100000000),400, 300, AlertMessageBox,true);
});
//电子签名身份认证
function SignValidate(){
         showPopWin('电子签名密码','../../../inc/SignValidate.aspx?Operate=Approve&file=<%=ReportPath %>&Number=<%=number %>&rand'+rand(100000000),400, 300, AlertMessageBox,true);
}

//签入电子签名
function SubmitReportToNext(){
   showPopWin('批准通过','../../../inc/SubmitReport.aspx?CMD=Approve&DetectID=<%=DetectID %>&UserID=<%=userid %>&rand'+rand(100000000),400, 300, AlertMessageBox,true);
}	
...
　　　　return Math.ceil(rnd()*number); 
    ...
    function HideAndBackToDefault(show){
       hidePopWin(show);
       window.location.href='http://'+window.location.host + "/Manager/Module/ShanliTech_HLD/DetectApprove/DefaultApprove.aspx";
       window.close();
    }
