<%@ Page Language="C#" MasterPageFile="~/Manager/MasterPage/PageTemplate.Master" AutoEventWireup="True" CodeBehind="Doc_Online_Approve.aspx.cs" Inherits="FrameWork.web.Manager.Module.ShanliTech_HLD.DeviceDetect.Doc_Online_Approve" %>
                    ...
                    <table height="26" cellspacing="0" cellpadding="0" width="100%" border="0" >
                        <tr>
                            <td>
                                ...
                                <asp:Button OnClick="Button10_Click" Text="批准通过并签名" id="Button10" runat="server" />
                                <asp:Button OnClick="Button4_Click" Text="批准不通过并签名" id="Button4" runat="server" />
...
//          showwait();	
   ...
   document.all.FramerControl1.HttpInit();
    ...
    document.all.FramerControl1.HttpPost("http://"+serverpath+"/Manager/Module/ShanliTech_HLD/DeviceDetect/Doc_Online_Save.aspx?file=<%=forfile %>&number=<%=number %>&forname=<%=forname %>");
} 
...
function OpenSealToWeb(signfile){
   ...
   var btn='<%=btnId %>';
   if(btn=='10')
        showPopWin('批准通过并提交批准','SubmitReport.aspx?CMD=Approve&DetectID=<%=DetectID %>&UserID=<%=userid %>&rand'+rand(100000000),400, 300, AlertMessageBox,true);
   else if(btn=='4')
        showPopWin('批准不通过原因说明','NoPass.aspx?CMD=Approve&DetectID=<%=DetectID %>&UserID=<%=userid %>&rand'+rand(100000000),400, 300, AlertMessageBox,true);
}	
