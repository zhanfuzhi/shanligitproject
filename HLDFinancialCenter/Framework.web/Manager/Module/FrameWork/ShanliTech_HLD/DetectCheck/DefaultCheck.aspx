<%@ Page Language="C#" MasterPageFile="~/Manager/MasterPage/PageTemplate.Master" AutoEventWireup="True"
    Codebehind="DefaultCheck.aspx.cs" Inherits="ShanliTech_HLD_Business.Web.Module.ShanliTech_HLD_Business.DetectCheck.DefaultCheck"
    Title="无标题页" %>
...
.td_input
{
    ...
    <FrameWorkWebControls:HeadMenuWebControls ID="HeadMenuWebControls1" runat="server" HeadHelpTxt="<a target='_blank' href='../../../Help/HelperDocument/DeviceDetectApp-InfoSecurity/DeviceDetectHelp.aspx#p9'>帮助</a>"
         HeadOPTxt="检定核验列表" HeadTitleTxt="检定核验">
    ...
    <FrameWorkWebControls:TabOptionWebControls ID="TabOptionWebControls1" runat="server">
        <FrameWorkWebControls:TabOptionItem ID="TabOptionItem1" runat="server" Tab_Name="证书检定核验列表">
            <asp:GridView ID="GridView1" runat="server" OnSorting="GridView1_Sorting" OnRowCreated="GridView1_RowCreated" >
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="操作">
                        <ItemTemplate>
                            <%--<a href="Doc_Online_Check.aspx?DetectID=<%#Eval("ID") %>&VerifierID=<%#Eval("VerifierID") %>&">进行核验</a>--%>
                            <%#GetOperateUrl(Convert.ToInt32(Eval("ID")), Convert.ToInt32(Eval("VerifierID")), Convert.ToInt32(Eval("VerifyFlag")), Eval("ReportPath").ToString(), Convert.ToInt32(Eval("ApproverID")), Convert.ToInt32(Eval("ApproveFlag")))%>
                            <a href="Doc_Online_Check.aspx?CMD=ViewDoc&ReportPath=<%#Eval("ReportPath") %>">查看证书</a>
                        </ItemTemplate>
                    </asp:TemplateField>
        ...
        <FrameWorkWebControls:TabOptionItem ID="TabOptionItem3" runat="server" Tab_Name="记录文档核验列表">
            <asp:GridView ID="GridView2" runat="server" OnRowCreated="GridView2_RowCreated" >
                <Columns>
                    ...
                    <asp:TemplateField HeaderText="操作">
                        <ItemTemplate>
                            <a href="javascript:showPopWin('核验通过','../../../inc/SignDetectRecord.aspx?DetectID=<%#Eval("ID") %>&file=<%#Eval("DetectRecordPath") %>&rand'+rand(100000000),400, 180, AlertMessageBox,true);">核验通过</a>
                            <a href="javascript:returnDetectRecord(<%#Eval("ID") %>)">核验驳回</a>
                            <a href="Doc_Online_Check.aspx?CMD=ViewDoc&ReportPath=<%#Eval("DetectRecordPath") %>">查看记录文档</a>
                        </ItemTemplate>
                    </asp:TemplateField>
                </Columns>
            </asp:GridView>
...
　　　　return Math.ceil(rnd()*number); 
    ...
    function returnDetectRecord(id){
        if(confirm("确定此操作吗？")){
            window.location.href = '../../../inc/ReturnDetectRecord.aspx?DetectID='+id;
        }
    }
    </script>
