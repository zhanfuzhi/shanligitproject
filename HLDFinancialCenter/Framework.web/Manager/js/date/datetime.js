...
function writeIframe() {
    ...
    for (var x = 0; x < 5; x++) {
        strIframe += "<tr>";
        for (var y = 0; y < 7; y++) strIframe += "<td class=out width=14% align=center id='meizzDay" + (x * 7 + y) + "'></td>"; strIframe += "</tr>";
    }
    strIframe += "<tr>";
...
var timeState="0";
function hiddenCalendar() {//加的判断条件是为了实现轨迹回放时时间的默认
    //if(timeState=="start"){//注释,修改BUG422轨迹回放时，不能选择回放的时间段 editor:liyansheng
//    WebCalendar.objExport.value=WebCalendar.objExport.value.split(" ")[0]+" 00:00";
//}else if(timeState=="end"){
//    WebCalendar.objExport.value=WebCalendar.objExport.value.split(" ")[0]+" 23:59";
//}
